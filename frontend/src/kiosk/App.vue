 ...
 *
 * Соседние страницы нужны собранными заранее: ближнюю видно в жёлобе подсказки, и
 * она приезжает под палец в первом же движении, а вторая страховает прыжок — номер
 * страницы меняется скачком после поиска и набора кода, не только стрелкой пейджера.
 * Дальние держат только своё место — пустой блок той же ширины, чтобы сдвиг ленты
 * считался как прежде.
 */
const RIBBON_WINDOW = 2
...
/**
 * Ко всем товарам: снимает набранный код, закрывает обе панели ввода и уходит к
 * началу каталога. Клавиатуру закрывает сам, а не через «касание мимо»: то
 * срабатывало на `pointerdown` и уводило клик другой кнопке.
 */
...
function onPointerDown(event: PointerEvent) {
  ...
  // Слот действия сам знает, что делать: «Усі товари» снимает набор, «ПОШУК товару»
  // открывает клавиатуру. Закрывать панели ввода здесь нельзя — слот перерисуется,
  // надпись под пальцем сменится, и `click` уйдёт уже другой кнопке: нажав
  // «Усі товари», покупатель попадал в поиск.
  ...
  //
  // Карточка — та же беда, но дороже: закрытие возвращает шапку и итоговую панель,
  // сетка уезжает вниз на 123 px (замер на 1920x1080), и `click` приходит уже по
  // другой карточке. Печать идёт от касания, поэтому покупатель получал этикетку на
  // чужой товар — или не получал ничего, если под пальцем оказывался промежуток.
  ...
  <el-config-provider :locale="elementLocale(locale)">
    ...
    <div class="kiosk" :class="{ 'hushed-scale': keyboardOpen }" :style="uiScales">
      ...
      <section class="main" :class="{ 'kb-open': keyboardOpen }">
        ...
        <!-- Пока набирают код, строка головы не нужна: набранное видно на самом
             блоке цифр, возврат ко всем товарам стоит в нижней панели, а поле
             поиска в этот момент всё равно закрыто. Освободившееся место уходит
             карточкам — ради них код и набирают. -->
        <div class="head-row" :class="{ hushed: showNumpad }">
...
/* Пока набирают код, строка пустеет и схлопывается: набранное видно на самом
   блоке цифр, возврат ко всем товарам стоит в нижней панели, а поле поиска в этот
   момент и так закрыто. Прячем содержимое, а не строку: убери её из разметки —
   и ряды сетки колонки сдвинутся, блок карточек потеряет свой `1fr`, а его
   заберёт пейджер. */
.head-row.hushed > * {
  display: none;
}
