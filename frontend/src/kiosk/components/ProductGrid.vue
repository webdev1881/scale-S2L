...
.photo img {
  ...
  width: 100%;
  height: 100%;
  /* Снимок всегда вписан целиком (`contain`), а размер задаёт один масштаб:
     100 % — фотография помещается в карточку без обрезки, больше — растёт и
     обрезается краями карточки, меньше — стоит с полями. Раньше способ
     вписывания переключался с `contain` на `cover` на пороге, и ровно там
     картинка скачком вырастала и обрезалась: соседние значения настройки
     давали разный вид, хотя разница между ними — один процент. */
  object-fit: contain;
