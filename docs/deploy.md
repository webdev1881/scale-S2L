...
Пошаговая инструкция «от коробки до выгрузки из 1С» для того, кто ставит прибор
в магазине, — `docs/new-device.md`. Здесь — справочник: что где лежит, обновление,
разбор неполадок.
...
| `S2L_ADMIN_PASSWORD` | пароль админки (логин `S2L_ADMIN_USER`, по умолчанию `admin`); пусто — без пароля | обязателен при туннеле |
...
настройки, каталог и чистку базы увидит любой, кто знает имя. Киоск и приём из 1С
...
/opt/s2l/
  ...
  data/
    settings.json настройки из админки (свои у каждого прибора, в гите только образец)
